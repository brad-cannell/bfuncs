...
tabstat <- function(x, digits = 3, stats = "default") {
  ...
  if ("ci" %in% stats) {                         # 95% Confidence Intervals
    lcl = round(mean(x2) - stats::qt(.975, n - 1) * stats::sd(x2) / sqrt(n), digits)
    ucl = round(mean(x2) + stats::qt(.975, n - 1) * stats::sd(x2) / sqrt(n), digits)
  ...
  if ("sd" %in% stats){                          # Standard deviation
    std_dev = round(stats::sd(x2), digits)
    df <- cbind(df, std_dev)
  }

  if ("var" %in% stats){                         # Variance
    variance = round(stats::var(x2), digits)
    df <- cbind(df, variance)
  }

  if ("cv" %in% stats){                          # Coefficient of variation
    coef_var = round(stats::sd(x2) / mean(x2), digits)  # (sd / mean)
    df <- cbind(df, coef_var)
  }

  if ("sem" %in% stats){                         # Coefficient of variation
    sem = round(stats::sd(x2) / sqrt(n), digits)        # (sd / sqrt(n))
    df <- cbind(df, sem)
  }
  ...
  if ("p1" %in% stats){                          # 1st percentile
    p1 = round(stats::quantile(x2, .01), digits)
    names(p1) <- NULL
    df <- cbind(df, p1)
  }

  if ("p5" %in% stats){                          # 5th percentile
    p5 = round(stats::quantile(x2, .05), digits)
    names(p5) <- NULL
    df <- cbind(df, p5)
  }

  if ("p10" %in% stats){                         # 10th percentile
    p10 = round(stats::quantile(x2, .1), digits)
    names(p10) <- NULL
    df <- cbind(df, p10)
  }

  if ("p25" %in% stats){                         # 25th percentile
    p25 = round(stats::quantile(x2, .25), digits)
    names(p25) <- NULL
    df <- cbind(df, p25)
  }
  ...
  if ("p75" %in% stats){                         # 75th percentile
    p75 = round(stats::quantile(x2, .75), digits)
    names(p75) <- NULL
    df <- cbind(df, p75)
  }

  if ("p90" %in% stats){                         # 90th percentile
    p90 = round(stats::quantile(x2, .90), digits)
    names(p90) <- NULL
    df <- cbind(df, p90)
  }

  if ("p95" %in% stats){                         # 95th percentile
    p95 = round(stats::quantile(x2, .95), digits)
    names(p95) <- NULL
    df <- cbind(df, p95)
  }

  if ("p99" %in% stats){                         # 99th percentile
    p99 = round(stats::quantile(x2, .99), digits)
    names(p99) <- NULL
    df <- cbind(df, p99)
  }

  if ("iqr" %in% stats){                         # Interquartile Range
    p75 = stats::quantile(x2, .75)
    p25 = stats::quantile(x2, .25)
  ...
  if ("q" %in% stats){                           # Shortcut for p25, p50, p75
    p25 = round(stats::quantile(x2, .25), digits)
    names(p25) <- NULL
    p50 = round(stats::quantile(x2, .50), digits)
    names(p50) <- NULL
    p75 = round(stats::quantile(x2, .75), digits)
    names(p75) <- NULL
